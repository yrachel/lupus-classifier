...
# def dpd_scorer(estimator, X, y, s):
...
def get_c_metrics(gs, X, y_test, s):
...
def get_r_metrics(gs, X, y_test, s):
    best    = gs.best_estimator_
    y_pred = best.predict(X)
...
def get_fair(gs, X, y, s):
    dpd_scorer = partial(demographic_parity_difference, sensitive_features=s)
    fair_scoring = {
        'accuracy': 'accuracy',
        'dpd': make_scorer(dpd_scorer, greater_is_better=False)
    }
    fair = ExponentiatedGradient(estimator=gs.best_estimator_, constraints=DemographicParity())
    fair_gs = GridSearchCV(fair, 
                           fairlearn_params, 
                           cv=cv_strat, 
                           scoring=fair_scoring, 
                           n_jobs=-1, 
                           verbose=1, 
                           return_train_score=True)
    fair_race_gs.fit(X, y)
...
for name, (model, param_grid) in clf_grids.items():
    ...
    fair = get_fair(gs, X_tr, y_train_c_res, s[["race", "sex"]])

    clf_results[name] = get_c_metrics(gs, X_te, y_test_c, s[["race", "sex"]])
    clf_results[name + " Fair"] = get_c_metrics(fair, X_te, y_test_c, s[["race", "sex"]])
...
X_train_r, X_test_r, y_train_r, y_test_r, s_train_r, s_test_r = train_test_split(
    X_sle, y_sledai, s, test_size=0.2, random_state=42
)
...
for name, (model, param_grid) in reg_grids.items():
    ...
    reg_results[name] = get_r_metrics(gs, X_te, y_test_r, s[["race", "sex"]])
    reg_results[name + " Fair"] = get_r_metrics(fair, X_te, y_test_r, s[["race", "sex"]])
...
plt.close()
# ─────────────────────────────────────────────
# EXPORT FOR DEMOGRAPHIC PORTION
# ─────────────────────────────────────────────
